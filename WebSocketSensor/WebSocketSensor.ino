...
static const uint32_t intervalTempHumidRead = 8000; // ms
...
void setup()
{
  ...
  pinMode(16, OUTPUT);          // GPIO16 (= pin D0) = blue LED
  digitalWrite(16, 1);          // inverted, i.e. light on when 0
...
void loop()
{
  ...
  if (secondChanged()) {
    ...
    sprintf(bufferDayName, "%s", str.c_str());
